...
class Web3ProviderPlugin extends EosTransitWeb3ProviderCore {
  constructor(
    ...
    this.popupSelectDesiredNetworkIfNeeded = this.popupSelectDesiredNetworkIfNeeded.bind(
      this
    );
  }
  ...
  async popupSelectDesiredNetworkIfNeeded(
    requiredNetwork: any & { chainId: number | string }
  ): Promise<void> {
    const { chainIdInt, chainIdHex } = this.getChainIdFromNetwork(
      requiredNetwork
    );
...
const web3WalletProviderPlugin = (args: Web3WalletProviderOptions) => {
  ...
  const pluginMetaData: PluginMetaData = {
    ...
    description:
      args?.description ||
      'Use Web3 Web Wallet to sign your Ethereum transactions',
